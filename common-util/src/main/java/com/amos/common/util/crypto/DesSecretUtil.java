...
public class DesSecretUtil {
    ...
    public static String encrypt(String sourceText, String salt) {
        if (sourceText == null || sourceText.length() == 0) {
            throw new RuntimeException("encrypt sourceText error (salt is blank)");
        }
        if (salt == null || salt.length() < CryptoConstant.MIN_KEN_LEN) {
            throw new RuntimeException("encrypt salt error (salt.length must greater than 8)");
        }
        try {
            // DES算法要求有一个可信任的随机数源
    ...
    public static String decrypt(String cipherText, String keyString) {
        if (cipherText == null || cipherText.length() == 0) {
            throw new RuntimeException("decrypt cipherText error (cipherText is blank)");
        }
        if (keyString == null || keyString.length() < CryptoConstant.MIN_KEN_LEN) {
            throw new RuntimeException("decrypt salt error (salt.length must greater than 8)");
        }
        try {
    ...
    private static int getHexValue(char paramChar) {
        if ((paramChar >= CryptoConstant.CHAT_ONE) && (paramChar <= CryptoConstant.CHAT_NINE)) {
            return (paramChar - CryptoConstant.CHAT_ONE);
        }
        if ((paramChar >= CryptoConstant.CHAT_LOWER_A) && (paramChar <= CryptoConstant.CHAT_LOWER_F)) {
            return (paramChar - CryptoConstant.CHAT_LOWER_A + 10);
        }
        if ((paramChar >= CryptoConstant.CHAT_UPPER_A) && (paramChar <= CryptoConstant.CHAT_UPPER_F)) {
            return (paramChar - CryptoConstant.CHAT_UPPER_A + 10);
        }
